...
from database import (
    initialize_database,
    save_message,
    retrieve_conversations_by_filters,
    check_password,
)
# rest of your code
...
if check_password():
    ...
    if st.checkbox("Log for Text Entries"):
        ...
        if start_date > end_date:
            ...
            conversations = retrieve_conversations_by_filters(
                start_date, end_date, user_name, conversation_id
            )
            if conversations:
                ...
                for user_name, timestamp, conv_id, role, content in conversations:
                    ...
                    conversation_threads[conv_id].append(
                ...
                for conv_id, messages in conversation_threads.items():
                    with st.expander(f"Conversation ID: {conv_id}"):
                        for user_name, timestamp, role, content in messages:
                            ...
                            st.markdown(
                                f"**{role.capitalize()}** ({user_name} - {time_display}): {content}"
                            )
            else:
    ...
    if st.checkbox("Log for Voice entries"):
        ...
        min_duration = st.number_input(
            "Minimum Call Duration (seconds)", min_value=0, value=10
        )
        ...
        if agent_id:
            ...
            for convo in filtered_conversations:
                ...
                with st.expander(
                    f"Message Count: {convo['message_count']} || Date: {start_time}"
                ):
                    ...
                    if st.button(
                        f"View Details of {convo['conversation_id']}",
                        ...
                        if details:
                            ...
                            for message in transcript:
                                ...
                                st.write(
                                    f"**{role.capitalize()}** (At {time_in_call}s): {content}"
                                )
